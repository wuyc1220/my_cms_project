...
返回 `xml.etree.ElementTree.Element`，形如::
    ...
    <Object ElementType="Program" ID="123" Code="123" Action="REGIST">
...
def _make_object(element_type: ElementType, action: Action, obj_id: Any) -> Element:
    ...
    规范要求 ID 和 Code 同时存在且值相同：
        <Object ElementType="X" ID="Z" Code="Z" Action="Y"/>
    """
    return Element(
        "Object",
        {
...
def build_program_object(
    ...
    custom_field_i18n: dict | None = None,
    ...
    多语言字段：Name, ShortTitle, Description, OriginalName, SortName, Genre, Tags, CustomField
    """
    ...
    t_i18n = tag_i18n or {}
    cf_i18n = custom_field_i18n or {}

    _add_property(obj, "VodType", meta.vod_type if meta else None)
    _add_i18n_property(obj, "Name", content.title, i18n, "name", langs, primary_language)
    ...
    if license_data:
        ...
        _add_property(obj, "Downloadable", downloadable, attrs=dl_attrs if dl_attrs else None)
    ...
    _add_i18n_property(obj, "OriginalName", meta.original_name if meta else None, i18n, "original_name", langs, primary_language)
    ...
    _add_property(obj, "SectionsInfo", meta.sections_info if meta else None)
    ...
    if license_data:
        pb = license_data.get("preview_begin_time")
        pe = license_data.get("preview_end_time")
        _add_property(obj, "TrailerBegintime", pb)
        _add_property(obj, "TrailerEndtime", pe)

    if custom_fields:
        primary_cf = custom_fields.get("custom_field")
        if cf_i18n:
            for lang in langs:
                cf_val = cf_i18n.get(lang, primary_cf)
                _add_property(obj, "CustomField", cf_val, attrs={"Language": lang})
        else:
            _add_property(obj, "CustomField", primary_cf)
...
def build_movie_object(
    ...
    _add_property(obj, "FileURL", movie.file_path)
    ...
    _add_property(obj, "FileSize", movie.file_size)
    ...
    _add_i18n_property(obj, "Name", None, i18n, "name", langs, primary_language)
...
def build_series_object(
    ...
    _add_property(obj, "Type", content_type_name)
    _add_property(obj, "RatingLevel", meta.rating_level if meta else None, attrs={"Type": "RTUK"})
    _add_property(obj, "VolumnCount", meta.volume_count if meta else None)
    _add_property(obj, "SeriesType", meta.series_type if meta else None)
    _add_property(obj, "CDR_ID", meta.cdr_id if meta else None)
    ...
    _add_property(obj, "SubtitleLang", meta.subtitle_lang if meta else None)
    _add_property(obj, "Studio", meta.studio if meta else None)
    _add_property(obj, "SeriesOrdinal", meta.series_ordinal if meta else None)
    _add_property(obj, "ShowID", meta.show_id if meta else None)
    _add_property(obj, "Keywords", meta.keywords if meta else None)
...
def build_schedule_object(
    ...
    PPVEnable 带 PackageID/PreBuffer/PostBuffer/PurchaseBeginTime/PurchaseEndTime XML 属性
    """
    ...
    _add_property(obj, "ChannelID", channel_id or str(content.parent_id) if content.parent_id else None)
    _add_property(obj, "ChannelCode", channel_code or str(content.parent_id) if content.parent_id else None)
    _add_i18n_property(obj, "ProgramName", content.title, i18n, "name", langs, primary_language)
    _add_i18n_property(obj, "Genre", genre_name, g_i18n, "", langs, primary_language)
    _add_property(obj, "BeginTime", _fmt_dt(content.begin_time))
    _add_property(obj, "EndTime", _fmt_dt(content.end_time))
    _add_property(obj, "Status", "1" if (meta.status_flag if meta else False) else "0")
    _add_i18n_property(obj, "Description", meta.description if meta else None, i18n, "description", langs, primary_language)
    _add_property(obj, "CUTVEnable", "1" if content.cutv_enable else "0")
    _add_property(obj, "ProgramID", meta.program_id if meta else None)
    _add_property(obj, "ContentTier", meta.content_tier if meta and hasattr(meta, "content_tier") else None)
    _add_property(obj, "RatingLevel", meta.rating_level if meta else None, attrs={"Type": "RTUK"})
    _add_property(obj, "BroadcastType", meta.broadcast_type if meta else None)
    _add_i18n_property(obj, "Actor", None, a_i18n, "", langs, primary_language)
    _add_i18n_property(obj, "Director", None, d_i18n, "", langs, primary_language)
    _add_property(obj, "AudioLang", meta.audio_lang if meta else None)
    _add_property(obj, "SubtitleLang", meta.subtitle_lang if meta else None)
    ...
    if pictures:
        for pic in pictures:
            pic_type = 2 if pic.entity_type == "Schedule" else 1
            _add_property(obj, "PictureID", str(pic.id), attrs={"Type": str(pic_type)})
    ...
    _add_property(obj, "PPVEnable", "1" if ppv_enable else "0", attrs=ppv_attrs if ppv_attrs else None)
    ...
    if meta:
        ...
        _add_property(obj, "SeriesID", meta.series_id)
        _add_property(obj, "VolumnCount", meta.volume_count)
        _add_property(obj, "Sequence", meta.sequence)
        _add_property(obj, "SeriesOrdinal", meta.series_ordinal)
        _add_property(obj, "ShowID", meta.show_id)
        ...
        _add_property(obj, "Studio", meta.studio)
...
def build_channel_object(
    ...
    _add_property(obj, "AudioLang", meta.audio_lang if meta else None)
    _add_property(obj, "SubtitleLang", meta.subtitle_lang if meta else None)
    _add_property(obj, "PPVEnable", meta.ppv_enable if meta else None)
...
def build_physical_channel_object(
    ...
    _add_property(obj, "ChannelID", pc.channel_id)
    _add_property(obj, "Name", pc.name)
    _add_property(obj, "Description", pc.name)
...
def build_category_object(
    ...
    if cat.parent_id is not None:
        obj.set("ParentCode", str(cat.parent_id))

    _add_property(obj, "ParentID", cat.parent_id)
    ...
    _add_property(obj, "JumpCategoryCode", cat.jump_category_code)

    if custom_fields:
        _add_property(obj, "PosterType", custom_fields.get("poster_type"))
        _add_property(obj, "Extendinfo", custom_fields.get("extendinfo"))
...
def build_cast_role_map_object(rm: CastRoleMap, action: Action, custom_fields: dict | None = None) -> Element:
    ...
    _add_property(obj, "CastID", rm.cast_id)
    _add_property(obj, "CastCode", rm.cast_id)
...
def build_picture_object(
    ...
    picture_type = 2 if pic.entity_type == "Schedule" else 1
