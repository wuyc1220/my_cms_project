"""
ContentPublish 定时任务业务逻辑。

扫描 publish_task 中到期的定时发布计划并逐条执行。
...
async def _get_max_retry(db: AsyncSession) -> int:
    """读取 ContentPublish 任务配置的最大重试次数。读不到时默认 0（不重试）。"""
    row = (
        await db.execute(
            select(ScheduledTask.retry_count).where(
                ScheduledTask.task_type == "ContentPublish",
...
async def run_content_publish(db: AsyncSession) -> dict | None:
    """
    扫描到期的定时发布计划并逐条执行。
    ...
    """
    logger.info(f"[ContentPublish] ========== 任务开始 ==========")
    max_retry = await _get_max_retry(db)
    now = datetime.now()
    logger.info(f"[ContentPublish] 最大重试次数={max_retry}, 当前时间={now}")
    logger.info(f"[ContentPublish] 扫描条件: task_type='publish', execution_mode='plan', scheduled_time<={now}, is_deleted=False")
    ...
    if not rows:
        logger.info(f"[ContentPublish] 无待处理任务，任务结束")
    ...
    for idx, task in enumerate(rows, 1):
        ...
        try:
            ...
            if refreshed is None:
                failed += 1
                logger.error(f"[ContentPublish] [{idx}/{len(rows)}] 任务 ID={task_id} 执行后无法获取刷新状态")
                ...
                if (refreshed.retry_attempts or 0) >= max_retry:
                    exhausted += 1
                    logger.warning(f"[ContentPublish] [{idx}/{len(rows)}] 任务 ID={task_id} 已达到最大重试次数 {max_retry}")
        except Exception as exc:  # noqa: BLE001
            logger.exception(f"[ContentPublish] [{idx}/{len(rows)}] 任务 {task_id} 执行异常：{exc}")
            try:
                ...
                if fresh is not None:
                    ...
                    await db.commit()
                    logger.error(f"[ContentPublish] [{idx}/{len(rows)}] 任务 ID={task_id} 异常已记录，重试次数={fresh.retry_attempts}/{max_retry}")
                    if (fresh.retry_attempts or 0) >= max_retry:
                        exhausted += 1
                        logger.warning(f"[ContentPublish] [{idx}/{len(rows)}] 任务 ID={task_id} 已达到最大重试次数 {max_retry}")
            except Exception:  # noqa: BLE001
                await db.rollback()
                logger.error(f"[ContentPublish] [{idx}/{len(rows)}] 任务 ID={task_id} 异常回滚失败")
            failed += 1
